...
Output ONLY valid JSON, no markdown, no preamble, exactly this shape:
{
  ...
  "walkAway": "the SMART maximum a BUYER should pay to still profit on resale. HARD RULE: the walk-away must sit AT OR BELOW the LOW end of the value range — never above it. A buyer must pay less than the item's worth to make money reselling; a walk-away higher than value-low is a logic error and is forbidden. Set it a sensible margin below value-low (typically 50-75% of the value low end for resale headroom), but not an insulting lowball that loses the item. If you give walk-away as a range, its HIGH end must not exceed the value LOW end.",
  "scenarios": [
